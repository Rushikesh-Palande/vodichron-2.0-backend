/**
 * Get Leave Balance Controller
 * =============================
 * Thin wrapper controller for Express routes.
 * Delegates to the service layer for business logic.
 * 
 * Purpose:
 * - Handle HTTP requests for fetching employee leave balance
...
/**
 * Get Leave Balance - Express Controller
 * =======================================
 ...
 * 
 * Response:
 * - 200: Success with leave balance breakdown
 * - 401: Unauthorized (not authenticated)
 * - 403: Forbidden (employee viewing another's balance)
 * - 500: Internal server error
 */
export async function getLeaveBalanceExpressController(req: Request, res: Response) {
  try {
    logger.info('📥 Get leave balance request received (Express)', {
      userId: (req as any).user?.uuid,
      employeeId: req.params.employeeId,
      endpoint: '/employee-leaves/:employeeId/balance'
    });
    ...
    const input: GetLeaveBalanceInput = {
      employeeId: req.params.employeeId,
    ...
    logger.info('✅ Leave balance fetched successfully (Express)', {
      employeeId: req.params.employeeId,
      year: result.year,
      userId: user.uuid
    });
    ...
    logger.error('💥 Get leave balance controller error', {
      type: 'GET_LEAVE_BALANCE_CONTROLLER_ERROR',
      employeeId: req.params.employeeId,
      error: error?.message,
      stack: error?.stack
    });
