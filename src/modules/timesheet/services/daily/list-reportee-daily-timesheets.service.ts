/**
 * List Reportee Daily Timesheets Service
 * =======================================
 * Business logic layer for managers/HR to view reportee daily timesheets
 * 
 * Based on weekly reportee timesheet service pattern
 * 
 * Responsibilities:
...
import { 
  getPaginatedAllDailyTimesheet,
  getPaginatedReporteeDailyTimesheet 
} from '../../stores/daily/list-reportee.store';
...
/**
 * Get Reportee Daily Timesheets Service
 * ======================================
 * 
 * Fetches paginated list of daily timesheets for reportees
 * 
 * Authorization Rules:
 ...
 * @returns Array of daily timesheets with data wrapper
 */
export async function getReporteeDailyTimesheets(
  pagination: { page?: number; pageLimit?: number },
  filters: DailyTimesheetFilters | undefined,
  user: UserContext
): Promise<{ data: any[] }> {
  const timer = new PerformanceTimer('getReporteeDailyTimesheets_service');
  
  try {
    logger.info('👥 Fetching reportee daily timesheets', {
      requestedBy: user.uuid,
      requestedByRole: user.role,
      pagination,
      filters,
      operation: 'getReporteeDailyTimesheets'
    });
    ...
    if (!isHROrSuperUser && !isManagerOrDirector) {
      ...
      logSecurity('GET_REPORTEE_DAILY_TIMESHEETS_ACCESS_DENIED', 'high', {
        userRole: user.role,
    ...
    if (isHROrSuperUser) {
      ...
      logger.debug('🔍 Fetching all daily timesheets (HR/Super User)', {
        userId: user.uuid,
        page,
        pageLimit,
        filters
      ...
      timesheets = await getPaginatedAllDailyTimesheet(
        filters || {},
        user.uuid,
        page,
        pageLimit
      ...
      logger.info('✅ All daily timesheets fetched (HR/Super User)', {
      ...
      logger.debug('🔍 Fetching reportee daily timesheets (Manager/Director)', {
        userId: user.uuid,
        page,
        pageLimit,
        filters
      ...
      timesheets = await getPaginatedReporteeDailyTimesheet(
        filters || {},
        user.uuid,
        page,
        pageLimit
      ...
      logger.info('✅ Reportee daily timesheets fetched (Manager/Director)', {
        count: timesheets.length,
        requestedBy: user.uuid,
        role: user.role
      });
    ...
    logger.info('✅ Reportee daily timesheets fetched successfully', {
      count: timesheets.length,
      page,
      pageLimit,
      requestedBy: user.uuid,
    ...
    logSecurity('GET_REPORTEE_DAILY_TIMESHEETS_SUCCESS', 'low', {
      count: timesheets.length,
      userRole: user.role,
      duration
    }, undefined, user.uuid);
    ...
    logSecurity('GET_REPORTEE_DAILY_TIMESHEETS_ERROR', 'critical', {
      error: error.message,
      userRole: user.role,
      duration
    }, undefined, user.uuid);
    ...
    throw new TRPCError({
      ...
      message: 'Failed to fetch reportee daily timesheets. Please try again later.'
    });
  }
}
