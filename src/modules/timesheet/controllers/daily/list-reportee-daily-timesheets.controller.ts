/**
 * List Reportee Daily Timesheets Controller
 * ==========================================
 * Thin wrapper controller for Express routes.
 * For managers/HR to view reportee daily timesheets.
 */
...
/**
 * List Reportee Daily Timesheets - Express Controller
 * ====================================================
 * 
 ...
 * 
 * @returns {Promise<Response>} JSON response with array of reportee daily timesheets
 * 
 ...
 * 
 * @description
 * - HR/SuperUser: Get ALL employee daily timesheets
 * - Manager/Director: Get reportee daily timesheets only
 */
export async function listReporteeDailyTimesheetsExpressController(req: Request, res: Response) {
  try {
    const { pagination, filters } = req.body;

    logger.info('📥 List reportee daily timesheets request received (Express)', {
      userId: (req as any).user?.uuid,
      userRole: (req as any).user?.role,
      endpoint: '/timesheet/daily/reportee'
    });
    ...
    logger.error('💥 List reportee daily timesheets controller error', {
      type: 'LIST_REPORTEE_DAILY_TIMESHEETS_CONTROLLER_ERROR',
      error: error?.message
    });
    ...
    return res.status(statusCode).json({
      success: false,
      message: error.message || 'Failed to fetch reportee daily timesheets',
      timestamp: new Date().toISOString()
    });
  }
}
